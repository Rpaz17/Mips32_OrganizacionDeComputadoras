...
void DataMemory::connectVGA(VGAFramebuffer *framebuff)
{
...
void DataMemory::storeVGAHalf(uint32_t addr, uint16_t value)
{
    ...
    uint8_t fg = static_cast<uint8_t>((value >> 8) & 0x00FF);
    uint8_t bg = static_cast<uint8_t>((value >> 12) & 0x00FF);

    vgaFrameBuffer->writePixel(row, col, ch, fg, bg);
    vgaFrameBuffer->commitFrame();
}
...
uint32_t DataMemory::loadWord(uint32_t address) const
{
    ...
    return static_cast<uint16_t>(memory[offs]) |
           (static_cast<uint16_t>(memory[offs + 1]) << 8) |
           (static_cast<uint16_t>(memory[offs + 2]) << 16) |
           (static_cast<uint16_t>(memory[offs + 3]) << 24);
}
